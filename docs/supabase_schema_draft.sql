...
create table public.role_verification_details (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references public.profiles(id) on delete cascade,
  role public.user_role not null,
...
create index role_verification_details_user_id_idx on public.role_verification_details(user_id);
create index role_verification_details_role_idx on public.role_verification_details(role);
...
on public.role_verification_details for select
to authenticated
...
on public.role_verification_details for insert
to authenticated
...
on public.role_verification_details for update
to authenticated
using (user_id = (select auth.uid()))
with check (user_id = (select auth.uid()) and role in ('merchant', 'delivery_agent'));
