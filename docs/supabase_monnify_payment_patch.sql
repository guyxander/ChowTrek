-- ChowTrek Monnify payment patch.
-- Run after the wallet patch. This keeps old card rows readable while making
-- Monnify the active card provider.

alter type public.payment_mode add value if not exists 'monnify';
...
alter table public.wallet_top_up_requests
  ...
  check (provider in ('monnify', 'quickteller', 'flutterwave'));
...
alter table public.transactions
  ...
  check (provider in ('monnify', 'quickteller', 'wallet', 'pay_on_delivery', 'flutterwave'));

create or replace function public.confirm_verified_monnify_payment(
...
begin
  ...
  if found then
    ...
    if merchant_owner_id is not null then
      ...
      values (
        ...
        merchant_settlement,
        'pending',
        'Verified Monnify card payment settlement'
      );
    end if;
  ...
  if found then
    ...
    values (
      ...
      'available',
      'Verified Monnify wallet top-up'
    );
  ...
  raise exception 'No pending Monnify order or wallet top-up found for reference %.', payment_reference;
end;
$$;

revoke all on function public.confirm_verified_monnify_payment(text, text) from public;
revoke execute on function public.confirm_verified_monnify_payment(text, text) from anon;
revoke execute on function public.confirm_verified_monnify_payment(text, text) from authenticated;
grant execute on function public.confirm_verified_monnify_payment(text, text) to service_role;
