...
module.exports = async function handler(request, response) {
  ...
  if (!SUPABASE_SERVICE_ROLE_KEY || !MONNIFY_API_KEY || !MONNIFY_SECRET_KEY) {
    response.status(503).json({
      ok: false,
      message: 'Monnify webhook verification is not configured on the server.'
    });
    return;
  }
  ...
  try {
    ...
    const isLive = String(MONNIFY_MODE).toUpperCase() === 'LIVE';

    if (signature && !isValidSignature(rawBody, String(signature))) {
      response.status(401).json({ ok: false, message: 'Invalid Monnify webhook signature.' });
      return;
    }

    if (isLive && !signature) {
      response.status(401).json({ ok: false, message: 'Missing Monnify webhook signature.' });
      return;
    }
    ...
    const reference = eventData.paymentReference || payload.paymentReference || '';

    if (eventType !== 'SUCCESSFUL_TRANSACTION') {
      response.status(200).json({ ok: true, ignored: true, message: `Ignored ${eventType || 'unknown'} event.` });
      return;
    }
    ...
    const verifiedData = verification.responseBody || {};
    const paymentStatus = String(verifiedData.paymentStatus || eventData.paymentStatus || '').toUpperCase();
    const amountPaid = Number(verifiedData.amountPaid || eventData.amountPaid || 0);
    ...
    const settlement = await callSettlementRpc(
      reference,
      verifiedData.transactionReference || eventData.transactionReference || null
    );

    response.status(200).json({
      ok: true,
      message: 'Monnify webhook payment verified.',
      reference,
      settlement
    });
  } catch (error) {
    response.status(500).json({
      ok: false,
      message: error instanceof Error ? error.message : 'Monnify webhook processing failed.'
    });
  }
};
...
function isValidSignature(rawBody, signature) {
  ...
  const hmacDigest = crypto.createHmac('sha512', MONNIFY_SECRET_KEY).update(rawBody).digest('hex');
  const legacyDigest = crypto
    .createHash('sha512')
    .update(`${MONNIFY_SECRET_KEY}${rawBody}`)
    .digest('hex');

  return safeEqual(normalizedSignature, hmacDigest) || safeEqual(normalizedSignature, legacyDigest);
}
